...
export class CheTaskProvider {
    ...
    async resolveTask(task: Task): Promise<Task> {
        ...
        const resultExecution: ShellExecution = {};
        ...
        const taskExecution = task.execution as ShellExecution;
        const command = taskExecution.command;
        if (!command) {
            return resultTask;
        }

        const unresolvedCommand = typeof command === 'string' ? command : command.value;
        resultExecution.command = await che.variables.resolve(unresolvedCommand);
